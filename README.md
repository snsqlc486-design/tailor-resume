...
최적화된 이력서를 **프로페셔널하게 디자인된 HTML 파일**로도 만들 수 있습니다.  
패키지 설치 없이, Claude가 직접 스타일이 적용된 HTML을 생성합니다.

**PDF가 필요하면?**  
생성된 `.html` 파일을 브라우저(Chrome 등)에서 열고 → `Cmd+P` (Mac) / `Ctrl+P` (Windows) → **PDF로 저장**
...
**Q: 이력서에 없는 내용이 추가되나요?**  
A: 아닙니다. 에이전트는 **절대 경험을 날조하지 않습니다.** 기존 경험을 직업 공고 맥락에 맞게 재구성하고 강조합니다.
