...
3. 매칭 분석          →  이력서 ↔ 공고 정직한 매칭 분석 (❌ 없는 기술 추가 금지)
4. 이력서 생성        →  원본 범위 내에서 최적화된 이력서 생성
5. 저장 및 리뷰       →  output/[회사명]_[직무명]_이력서.md로 저장
6. HTML 변환 (선택)   →  디자인 선호도 질문 → 맞춤 HTML 생성
...
A: 네! HTML 변환 시 에이전트가 컴러, 레이아웃, 폰트, 분량 등을 물어봅니다. `"추천대로"` 하면 기본 스타일로 진행됩니다.
